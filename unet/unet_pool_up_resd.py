...
def unet_pool_up_deep_2f2(cfg):
    # if cfg.model_filter is None:
        # cfg.model_filter = [64, 96, 128, 192]
    # cfg.model_filter = [64, 64, 64, 64, 64, 64, 64]
    cfg.model_filter = [48, 48, 48, 48, 48, 48, 48]
    ...
    locals()['pool0']=Input((cfg.row_in, cfg.col_in, dim_in))
    # locals()['pool0']=Conv2D(fs[0], (ks[0],ks[0]), activation=model_act, padding='same', kernel_initializer=init)(locals()['in0'])  # r,c,3

    for i in range(len(fs)):
        locals()['conv'+str(i)]=concatenate([Conv2D(int(fs[i]/2), (ks[2], ks[2]), activation=model_act, padding='same', kernel_initializer=init)\
                (Conv2D(fs[i], (ks[0], ks[0]), activation=model_act, padding='same', kernel_initializer=init)(locals()['pool'+str(i)])),\
                                             locals()['pool'+str(i)]], name='conv' + str(i), axis=concat_axis)
        if i < len(fs)-1:
    ...
    for i in range(len(fs)-2,-1,-1):
        ...
        locals()['decon'+str(i)] =concatenate([locals()['upsamp'+str(i)], Conv2D(int(fs[i]/2), (ks[1], ks[1]), activation=model_act, kernel_initializer=init, padding='same', name='decon'+str(i))\
            (Conv2D(fs[i], (ks[0], ks[0]), activation=model_act, kernel_initializer=init, padding='same')(locals()['upsamp'+str(i)]))], axis=concat_axis)
