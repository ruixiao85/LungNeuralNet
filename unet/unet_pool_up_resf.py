...
def unet_pool_up_res_2f1(cfg):
    ...
    for i in range(len(fs)):
        locals()['conv'+str(i)]=concatenate([locals()['pool'+str(i)],
                 Conv2D(fs[i], (ks[1], ks[1]), activation=model_act, padding='same', kernel_initializer=init)\
                (Conv2D(fs[i], (ks[0], ks[0]), activation=model_act, padding='same', kernel_initializer=init)(locals()['pool'+str(i)]))],
            name = 'conv' + str(i),axis=concat_axis)
        if i < len(fs)-1:
            locals()['pool' + str(i+1)] = MaxPooling2D((2, 2), strides=(2, 2), name='pool' + str(i+1))(locals()['conv'+str(i)])
