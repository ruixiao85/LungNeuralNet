...
class RoiAlign(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        _, indices     = retinanet.backend.top_k(scores, k=keras.backend.minimum(self.top_k, keras.backend.shape(boxes)[0]), sorted=False)
        ...
        for i in range(len(fpn)):
            ...
            indices = retinanet.backend.where(keras.backend.equal(target_levels, i))

            level_boxes          = retinanet.backend.gather_nd(boxes, indices)
            level_classification = retinanet.backend.gather_nd(classification, indices)
