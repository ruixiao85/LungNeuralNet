...
def mask(iou_threshold=0.5, mask_size=(28, 28)):
    ...
    def _mask(y_true, y_pred, iou_threshold=0.5, mask_size=(28, 28)):
        ...
        indices              = retinanet.backend.where(keras.backend.greater_equal(max_iou, iou_threshold))
        boxes                = retinanet.backend.gather_nd(boxes, indices)
        masks                = retinanet.backend.gather_nd(masks, indices)
        argmax_overlaps_inds = keras.backend.cast(retinanet.backend.gather_nd(argmax_overlaps_inds, indices), 'int32')
        ...
        masks = retinanet.backend.gather_nd(masks, label_indices)
