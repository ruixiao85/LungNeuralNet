...
from matterport_mrcnn import utils

class BaseNetM(Config):
    def __init__(self,is_train=None,num_classes=None,mini_mask_shape=None,out_mask_shape=None,
                 backbone=None,batch_norm=None,backbone_stride=None,pyramid_size=None,use_rpn_rois=None,
        ...
        self.use_rpn_rois=use_rpn_rois is use_rpn_rois is not None else True # False if you train with ROI generated by code rather than from rpn
        ...
        C1,C2,C3,C4,C5=self.convolution_backbone(input_image) # Bottom-up Layers (cnn backbone)

        P2,P3,P4,P5,P6=self.feature_pyramid_network(C2,C3,C4,C5) # Top-down Layers (fpn)
        ...
        rpn=self.region_proposal_network() # rpn model
        ...
        if self.is_train:
            ...
            rois,target_class_ids,target_bbox,target_mask=\
                DetectionTargetLayer(self.images_per_gpu,self.train_rois_per_image,self.mini_mask_shape,name="proposal_targets")\
                    ([target_rois,input_gt_class_ids,gt_boxes,input_gt_masks])
            # Network Heads
            mrcnn_class_logits,mrcnn_class,mrcnn_bbox=\
                fpn_classifier_graph(rois,mrcnn_feature_maps,input_image_meta,self.pool_size,self.num_classes,
            ...
            mrcnn_class_logits,mrcnn_class,mrcnn_bbox=\
                fpn_classifier_graph(rpn_rois,mrcnn_feature_maps,input_image_meta,self.pool_size,self.num_classes,
                                     train_bn=self.batch_norm,fc_layers_size=self.fc_layers_size)
            ...
            mrcnn_mask=build_fpn_mask_graph(detection_boxes,mrcnn_feature_maps,input_image_meta,self.mask_pool_size,
                                            self.num_classes,train_bn=self.batch_norm)
    ...
    def feature_pyramid_network(self,C2,C3,C4,C5):
    ...
    def region_proposal_network(self):
        feature_map=KL.Input(shape=[None,None,self.pyramid_size],name="input_rpn_feature_map")
        ...
        return KM.Model([feature_map],[rpn_class_logits,rpn_probs,rpn_bbox],name="rpn_model")
...
class ProposalLayer(KE.Layer):
    def __init__(self, proposal_count, nms_threshold, rpn_bbox_stdev, pre_nms_limit, images_per_gpu, **kwargs):
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    boxes = positive_rois
    if config.USE_MINI_MASK: # Transform ROI coordinates from normalized image space to normalized mini-mask space.
