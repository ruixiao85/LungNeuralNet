...
class MaskRCNN(BaseNetM):
    def __init__(self,is_train,dim_in=None, dim_out=None, **kwargs
        ):       # mode, config, model_dir):
        ...
        super(MaskRCNN,self).__init__(dim_in=dim_in or (768,768,3),dim_out=dim_out or (768,768,1),**kwargs)
        self.is_train=is_train

        # self.fs=filters or [64,128,256,512,1024]
        # self.ps=poolings or [2]*len(self.fs)

        # self.num_classes=1 # if one at a time
        # self.meta_shape=[1+3+3+4+1+self.num_classes] # last number is NUM_CLASS
        # self.mini_mask_shape=[56,56,None]  # target shape (downsized) of instance masks to reduce memory load.
        # self.out_mask_shape=[28,28]
        # self.backbone="resnet50" # "resnet101"
        # self.stage_5=True
        # self.batch_norm=False # default to false since batch size is often small
        # self.pyramid_size=256 # Size of the top-down layers used to build the feature pyramid
        # self.rpn_anchor_ratio=[0.5,1,2] # Ratios of anchors at each cell (width/height) 1=square 0.5=wide
        # self.rpn_anchor_stride=1 # 1=no-skip cell 2=skip-one
        # self.pre_nms=6000 # ROIs kept after tf.nn.top_k and before non-maximum suppression
        # self.post_mns_train, self.post_nms_predict=2000, 1000 # ROIs kept after non-maximum suppression (train and predict)
        # self.rpn_nms_threshold=0.7 # Non-max suppression threshold to filter RPN proposals. larger=more propsals.
        # self.rpn_bbox_stdev=np.array([0.1,0.1,0.2,0.2]) # Bounding box refinement standard deviation for RPN and final detections.
        # self.image_per_gpu=1 # 1024x1024 needs 6GB for GPU
        # self.use_rpn_rois=True # default to true false if you want to train the head branches on ROI generated by code rather than the ROIs from the RPN.
    ...
    def build(self):
        ...
        # Bottom-up Layers (shared convolutional layers)
        _, C2, C3, C4, C5 = resnet_graph(input_image, self.backbone, self.stage_5, self.batch_norm)
        # Top-down Layers
        P5 = KL.Conv2D(self.pyramid_size, (1, 1), name='fpn_c5p5')(C5)
        ...
        # Anchors
        if self.is_train:
            anchors = self.get_anchors([self.row_in,self.col_in,self.dep_in])
        ...
        # RPN Model
        rpn = build_rpn_model(self.rpn_anchor_stride, len(self.rpn_anchor_ratio), self.pyramid_size)
        layer_outputs=[rpn([p]) for p in rpn_feature_maps] # Loop through pyramid layers
        # Concatenate layer outputs (list of lists of level outputs -> list of lists of outputs across levels)
        ...
        rpn_rois = ProposalLayer(proposal_count=self.post_mns_train if self.is_train else self.post_nms_predict,
                                 nms_threshold=self.rpn_nms_threshold, cfg=self, name="ROI")([rpn_class, rpn_bbox, anchors])
        ...
        if self.is_train:
            ...
            if self.use_rpn_rois:
                target_rois = rpn_rois
            else:
                input_rois = KL.Input(shape=[self.post_mns_train, 4], name="input_roi", dtype=np.int32) # Ignore predicted ROIs and use provided ROIs
                target_rois = KL.Lambda(lambda x: norm_boxes_graph(x, K.shape(input_image)[1:3]))(input_rois) # Normalize coordinates
            ...
            rois, target_class_ids, target_bbox, target_mask = DetectionTargetLayer(self.images_per_gpu, self.train_rois_per_image, self.mask_shape, name="proposal_targets")([
