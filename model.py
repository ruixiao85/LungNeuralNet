...
def get_unet3(img_rows, img_cols, dim=1):
    inputs = Input((img_rows, img_cols, dim))
    ...
    outputs = Conv2D(num_class, (1, 1), activation='sigmoid')(decon1)

    model = Model(inputs=inputs, outputs=outputs)
    return model


def get_unet5(img_rows, img_cols, dim=1):
    inputs = Input((img_rows, img_cols, dim))
...
def get_unet6(img_rows, img_cols, dim=1):
    inputs = Input((img_rows, img_cols, dim))
