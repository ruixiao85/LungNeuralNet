...
def unet_pool_up_4_dure(img_rows, img_cols, dim_in, dim_out, act_fun='elu', out_fun='sigmoid', init='he_normal'):
    name="_unet_pool_up_4_dure"
    ...
    conv2 = Conv2D(f2, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv2)
    pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)
    conv3 = Conv2D(f3, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(pool2)
    conv3 = Conv2D(f3, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv3)
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
    conv4 = Conv2D(f4, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(pool3)
    conv4 = Conv2D(f4, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv4)
    ...
    decon3 = Conv2D(f3, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(decon3)
    ...
    decon2 = Conv2D(f2, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(decon2)
...
def unet_pool_up_5_dure(img_rows, img_cols, dim_in, dim_out, ratio, act_fun='elu', out_fun='sigmoid', init='he_normal'):
    name="_unet_pool_up_5_d"+str(ratio)
    ...
    conv1 = Conv2D(f1, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(img_input)
    conv1 = Conv2D(int(1.0*f1/ratio), (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv1)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)
    conv2 = Conv2D(f2, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(pool1)
    conv2 = Conv2D(int(1.0*f2/ratio), (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv2)
    pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)
    conv3 = Conv2D(f3, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(pool2)
    conv3 = Conv2D(int(1.0*f3/ratio), (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv3)
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
    conv4 = Conv2D(f4, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(pool3)
    conv4 = Conv2D(int(1.0*f4/ratio), (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv4)
    pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)
    conv5 = Conv2D(f5, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(pool4)
    conv5 = Conv2D(int(1.0*f5/ratio), (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv5)
    ...
    decon4 = Conv2D(int(1.0*f4/ratio), (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(decon4)
    up_conv3 = UpSampling2D(size=(2, 2))(decon4)
    up3 = concatenate([up_conv3, Cropping2D(cropping=(get_crop_shape(conv3, up_conv3)))(conv3)], axis=concat_axis)
    decon3 = Conv2D(f3, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(up3)
    decon3 = Conv2D(int(1.0*f3/ratio), (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(decon3)
    up_conv2 = UpSampling2D(size=(2, 2))(decon3)
    up2 = concatenate([up_conv2, Cropping2D(cropping=(get_crop_shape(conv2, up_conv2)))(conv2)], axis=concat_axis)
    decon2 = Conv2D(f2, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(up2)
    decon2 = Conv2D(int(1.0*f2/ratio), (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(decon2)
    up_conv1 = UpSampling2D(size=(2, 2))(decon2)
    up1 = concatenate([up_conv1, Cropping2D(cropping=(get_crop_shape(conv1, up_conv1)))(conv1)], axis=concat_axis)
    decon1 = Conv2D(f1, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(up1)
    decon1 = Conv2D(int(1.0*f1/ratio), (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(decon1)
...
def unet_pool_up_6_dure(img_rows, img_cols, dim_in, dim_out, act_fun='elu', out_fun='sigmoid', init='he_normal'):
    name="_unet_pool_up_6_dure"
    ...
    pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)
    conv5 = Conv2D(f5, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(pool4)
    conv5 = Conv2D(f5, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv5)
    pool5 = MaxPooling2D(pool_size=(2, 2))(conv5)
    conv6 = Conv2D(f6, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(pool5)
    conv6 = Conv2D(f6, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv6)
    ...
    decon5 = Conv2D(f5, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(decon5)
    up_conv4 = UpSampling2D(size=(2, 2))(decon5)
    up4 = concatenate([up_conv4, Cropping2D(cropping=(get_crop_shape(conv4, up_conv4)))(conv4)], axis=concat_axis)
    decon4 = Conv2D(f4, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(up4)
    decon4 = Conv2D(f4, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(decon4)
    up_conv3 = UpSampling2D(size=(2, 2))(decon4)
    up3 = concatenate([up_conv3, Cropping2D(cropping=(get_crop_shape(conv3, up_conv3)))(conv3)], axis=concat_axis)
    decon3 = Conv2D(f3, (3, 3), activation=act_fun, padding='same', kernel_initializer=init)(up3)
    decon3 = Conv2D(f3, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(decon3)
...
def unet_pool_up_7_dure(img_rows, img_cols, dim_in, dim_out, act_fun='elu', out_fun='sigmoid', init='he_normal'):
    name="_unet_pool_up_7_dure"
    ...
    conv7 = Conv2D(f7, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(conv7)
    ...
    decon6 = Conv2D(f6, (1, 1), activation=act_fun, padding='same', kernel_initializer=init)(decon6)
